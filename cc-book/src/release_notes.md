...
- JVM and native TypeScript: the keystore now uses SQLite3 Multiple Ciphers instead of SQLCipher with a vendored
  OpenSSL. It reads and writes SQLCipher's format, so existing keystores open as before, and earlier CoreCrypto versions
  can still open keystores written by this one. Android and iOS keep SQLCipher.
