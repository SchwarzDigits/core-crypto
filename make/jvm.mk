...
# libraries are then copied to where a host build puts them, and checked.
JVM_LINUX_MANYLINUX ?= $(if $(CI),1,)
...
# place and check the glibc they need. A library that needs a newer one is removed.
jvm-linux-finish = $(if $(jvm-linux-manylinux),\
	...
	scripts/in-manylinux.sh $(1) scripts/check-max-glibc.sh $(3) 2.28 || { rm -f $(3); exit 1; })

JVM_LINUX_LIB := target/x86_64-unknown-linux-gnu/$(RELEASE_MODE)/libcore_crypto_ffi.$(LIBRARY_EXTENSION)
jvm-linux-deps := $(RUST_SOURCES) make/jvm.mk scripts/in-manylinux.sh scripts/check-max-glibc.sh
