...
`target/digits/`. The Rust version is the channel of `rust-toolchain.toml`. The script records the commit it built from
in `target/digits/built/<target>`.
...
`release/package.sh` builds the Maven publication into `target/digits/maven/` and installs it into the local Maven
...
The script downloads Wire's files of the same version from Maven Central into `target/digits/wire/` and checks them
...
end, `release/check.py` compares the publication with Wire's release and checks the POM and the SBOM.

## Kotlin Multiplatform

`release/publish-kmp.sh` publishes `schwarz.opensource.natrium:core-crypto-kmp` with Wire's Gradle build of this branch
into `target/digits/maven/`; `--m2` also copies it into `~/.m2/repository`:
...
`release/third-party.py` lists what an artifact contains:
...
```sh
release/third-party.py notices android 10.5.2-digits.1 THIRD_PARTY_NOTICES.txt
```
...
1. Sign and bundle, in a terminal, since GnuPG asks for the passphrase:
   ...
   checksums, runs `check.py --signed` and writes `target/digits/core-crypto-jvm-<version>-bundle.zip`.
